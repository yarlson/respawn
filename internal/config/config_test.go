...
`
		...
		assert.Equal(t, "claude-3-sonnet", cfg.Backends["opencode"].Models.Fast.Name)
		assert.Equal(t, "high", cfg.Backends["opencode"].Models.Fast.Variant)
		assert.Equal(t, "claude-3-opus", cfg.Backends["opencode"].Models.Slow.Name)
